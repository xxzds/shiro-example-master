...
public class OrganizationDaoImpl implements OrganizationDao {
    ...
    public void move(Organization source, Organization target) {
        ...
        String moveSourceDescendantsSql = "update sys_organization set parent_ids=concat(?, substring(parent_ids, length(?)+1)) where parent_ids like ?";
        jdbcTemplate.update(moveSourceDescendantsSql, target.makeSelfAsParentIds(), source.getParentIds(), source.makeSelfAsParentIds() + "%");
    }
}
